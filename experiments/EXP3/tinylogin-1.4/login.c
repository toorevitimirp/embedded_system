...
extern int login_main(int argc, char **argv)
{
	...
	while (( flag = getopt(argc, argv, "f:h:pu:s:")) != EOF ) {
		switch ( flag ) {
		case 'p':
			opt_preserve = 1;
			break;
		...
		case 'u':
			if (strlen(optarg) > 32) {
				printf("The username is too long,make it less than 32 bytes\n");
				exit(EXIT_FAILURE);
			}
			uflg++;
			safe_strncpy(username, optarg, USERNAME_SIZE);
		...
		case 's':
			if (strlen(optarg) > 32) {
				printf("The passwd is too long,make it less than 32 bytes\n");
				exit(EXIT_FAILURE);
			}
			sflg++;
			safe_strncpy(cipher, optarg, 32);
			break;
#endif
		default:
			show_usage ( );
...
#ifdef CONFIG_HHTECH
	if ( !(uflg && sflg) && (!isatty(0) || !isatty(1) || !isatty(2))) {
#else
...
hhcn:
	...
	while ( 1 ) {
		...
		if (sflg && uflg && (( strcmp ( pw-> pw_passwd, "x" ) == 0 ) || ( strcmp ( pw-> pw_passwd, "*" ) == 0 ))) {
		...
		if (sflg && uflg && strcmp(pw_encrypt(cipher,correct),correct)== 0)
		{
...
auth_ok:
	...
	//HHTECH
	run_shell("/bin/sh", 1, 0, 0 );	/* exec the shell finally. */
