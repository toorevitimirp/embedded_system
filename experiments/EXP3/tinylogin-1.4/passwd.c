...
extern int passwd_main(int argc, char **argv)
{
	...
	if (pflg){
		...
		if (algo == 2) {
			crypt_pass = pw_encrypt(passwd_string, "$2$");
		} else
#endif
#ifdef TLG_FEATURE_MD5_PASSWORDS
		if (algo == 1) {
			crypt_pass = pw_encrypt(passwd_string, "$1$");
		} else
#endif
			crypt_pass = pw_encrypt(passwd_string, crypt_make_salt());
		//STRFCPY(crypt_passwd, crypt_pass);
		safe_strncpy(crypt_passwd, crypt_pass, sizeof(crypt_passwd));
	}
#endif
#ifdef CONFIG_HHTECH
	if (!(dflg || lflg || uflg)&&(pflg==0)) {
#else
	if (!(dflg || lflg || uflg)) {
#endif
		if (!amroot) {
