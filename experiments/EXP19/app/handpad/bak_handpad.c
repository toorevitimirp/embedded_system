...
{	
	...
	for(i=0;i<10;)
	{
	    ...
	    //printf("tmp x=%d,y=%d,i=%d\n",cBuffer.x,cBuffer.y,i);
	    if(cBuffer.pressure){
		    if(cBuffer.y == 1022 && cBuffer.y != ylast){
			    ylast=1022;
		    }else{
			x_sum += cBuffer.x;
			y_sum += cBuffer.y;
	...
	*x = (y_sum/10-380)*160/642;//(380-1022)
	*y = (x_sum/10-328)*127/432;//(328-760)
	//printf("x=%d,y=%d\n",*x,*y);
	return 1;	
}
...
void FindCandidate(char*buf,short*start,short*end)
{
	...
	len=strlen(buf);
	printf("FindCandidate:Current Buf:%s len=%d\n",buf,len);
	if(len>0)
	{
		if(!current)
		{
...
int main(void)
{
	...
	//printf("handpad:begin while(1)\n");
	buf[0]='z';buf[1]='a';buf[2]='n';buf[3]='g';
	//while(1)
	for(it=0;it<1;it++)
	{
