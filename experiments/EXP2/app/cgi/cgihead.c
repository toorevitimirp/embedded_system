...
int bohao(){
	...
	if(vfork() == 0){
	        execl("/cramfs/sbin/pppd","/cramfs/sbin/pppd","modem","/dev/ttyS1","57600","crtscts","connect","/cramfs/sbin/chat -v -f /etc/config/chat.ttySx","debug", "noauth", "noipdefault", "defaultroute",NULL);
	       // execl("/cramfs/sbin/pppd","/cramfs/sbin/pppd","modem","/dev/ttyS1","115200","crtscts","connect","/cramfs/sbin/chat -v -f /etc/config/chat.ttySx","debug", "noauth", "noipdefault", "defaultroute",NULL);
...
int setmac(){
	...
	snprintf(stringtowrite,37,"/cramfs/app/setmac %s %s %s %s %s %s",splitedstring.stringout[1],splitedstring.stringout[2],splitedstring.stringout[3],splitedstring.stringout[4],splitedstring.stringout[5],splitedstring.stringout[6]);
	...
	save();
	if(fork() == 0){
              execl("/cramfs/app/macset","/cramfs/app/macset",NULL);
...
int startbjdj(){
	...
	pid_t tmp;
	if((tmp=vfork()) == 0){
		execl("/cramfs/app/motor_test","/cramfs/app/motor_test",NULL);
...
int recorder(){
	...
	if(vfork() == 0){
	        execl("/cramfs/app/recorder","/cramfs/app/recorder","-n",stringtowrite,"&",NULL);
...
int playmtv(){
	...
	mount("/dev/sda1","/mnt","vfat",0xc0ed0000,NULL);
	sleep(3);
...
//	printf("<center>12345</center>");
	if(vfork() == 0){
//syslog("adasfafas");
	        execl("/cramfs/app/play","/cramfs/app/play",NULL);
